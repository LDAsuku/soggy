...
public:
  ...
  const char *find_content_type(const std::string &path); // soggy
...
// soggy
inline const char *Server::find_content_type(const std::string &path) {
  return detail::find_content_type(path, file_extension_and_mimetype_map_);
}
